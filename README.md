...
```mermaid
flowchart LR
	MC[兼容 Minecraft 协议的服务端] --> NET[client/network
TCP + 加密 + 压缩 + 分包]
...
4. [client/src/network/connection.rs](client/src/network/connection.rs) 处理 TCP 连接、登录/配置/游戏状态切换，以及加密和压缩外壳。
...
- 加密、压缩、读取网络帧等传输层逻辑保留在 client/network。
